...
(function($) {
    ...
    var commands = {
                    ...
                    $obj.find(options.jplayerId).jPlayer({
                        ...
                        ready: function (ev) {
                            ...
                            $(this).jPlayer("setMedia", {
                                "mp3" : 'http://' + location.host + '/' + options.soundfile + ".mp3"//,
                                //"m4a" : 'http://' + location.host + '/' + options.soundfile + ".m4a"
                                //"oga" : 'http://' + location.host + '/' + options.soundfile + ".ogg"
                            });
                        },
        ...
        play : function(options) {
			...
			//REMOVING THE OLD TIMEOUT + STOP PLAYER
			$('.'+opts.jplayerClass).jPlayer('stop');
			$.each(timeouts, function( key, timeout ) {
				clearTimeout(timeout);
			});
			$('.'+opts.highlightClass).removeClass(opts.highlightClass);
        ...
        stop : function() {
			var opts = $.fn.narrateify.defaults;
			$('.'+opts.jplayerClass).jPlayer('stop');
			$.each(timeouts, function( key, timeout ) {
				clearTimeout(timeout);
			});
    ...
    $.fn.narrateify.defaults = {
        ...
        padding           : 0,
        timings           : {},
        highlightClass    : 'active-phrase',
		jplayerClass	  : 'jplayer'
    };
